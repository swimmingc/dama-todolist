...
class scheduler:
    ...
    def __init__(self):
        ...
        #일정에 대한 연산을 위하여 last_number를 선언한다
    ...
    # 스케줄 업데이트를 위한 함수선언
    # 과제와 개인일정인지 구분한다. 
    # 입력방식은 과제/개인일정, 해야할 일 순서이고 이를 split을 이용하여 ,를 기준으로 구분한다
    def make_scedule(self):
        ...
        self.btnRead=tkinter.Button(self.window, height=1, width=10, text="입력", command=self.getTextInput,  bg = "black",fg = "white")
    ...
    def dama(self):
        ...
        #생성 이미지를 tkinter에서 인식할수 있도록 convert
...
#        except:
#            pass
#        self.window.after(30000, self.update_time)
    ...
    def show_sc_list(self):
        ...
        for i,value in enumerate(self.total):
            ...
            if self.total[i].types=='과제':
                ...
                radio1=tkinter.Radiobutton(self.window,  value=i+1, variable=self.radio_check[i], command=self.change_radio)
